...
__version__ = "0.2"
__email__ = "vlevorato@octo.com, amassiot@octo.com"
...
def mspe(y_true, y_pred):
    ...
    """
    return np.mean(np.ma.masked_invalid(np.power((y_true - y_pred) / (y_true * 1.0), 2)))
...
def mape(y_true, y_pred):
    ...
    """
    return np.mean(np.ma.masked_invalid(np.abs((y_true - y_pred) / (y_true * 1.0))))
